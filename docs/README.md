...
- 主要な設計方針は `decisions.md` に56件記録済み（うち2件は Superseded: `D-016` `D-033`）
- 満たすべき要件は `requirements.md` に28件（`R-001`〜`R-028`）
- 実装仕様書は `spec/` に4件（`001-schema.md` / `002-loader.md` / `004-leakage-test.md` / `012-data-quality.md`、いずれも Draft）
...
- `P-1` の未決は `Q-025`（混合効果モデルの実装）と `Q-028`（合成 fixture の規模）。`003-features.md` は未作成
