...
| ドキュメント | 役割 | 誰が読むか |
|---|---|---|
| [`requirements.md`](./requirements.md) | 満たすべき要件（`R-xxx`）。**まだ無い** | 完了判定をする人 |
...
- 主要な設計方針は `decisions.md` に18件記録済み（うち1件は Superseded）
- 着手の前提となる未決事項が `open-questions.md` に残っている（特に `Q-011` はデータ取得の前提条件）
