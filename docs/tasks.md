...
- [ ] 10年分の取り込みを完了し、`fail` が0件であることを確認する（`012-data-quality.md` / `Q-033`）
      **ブロック中: `Q-033`**（レース取り込みは完了・血統取得はバックグラウンドで取得中）
      ...
      進捗: レース取り込み完了（33,236レース・467,927出走行、`http_error`13件・`parse_error`0件）。`fail` 合計0件を確認済み。取り込み中に `payout_horses` で8件の違反を検出し、失格マーカー未対応（`Q-023`）が原因と判明・解決した（`D-034` 追記）。血統取得（新規馬）は継続中
