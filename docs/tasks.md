...
- [x] `class_weights`/カテゴリ丸め閾値のハイパーパラメータ探索を実装する（`D-081` `D-092` `D-051` `D-059` / `D-101`）
      ...
      完了（2026-08-24）: 実データの fold 0・fold 6（inner LogLoss平均）で12候補（`class_weights`4種 × `min_count`3種）を比較。`class_weights={"G1":10,"G2":4,"G3":2,"L":1.5}`・`min_count=20` を採用（`D-101` に詳細）。**ただし12候補の差はごくわずか（0.34%）で、`R-023` の未達（市場を約21.5%上回る）はこの2パラメータのチューニングだけでは説明できない**
...
- [ ] `F-902` の `k`（縮約強度）・`F-101` 等の窓 `N` のハイパーパラメータ探索を実装する（`D-051` `D-059` / `D-101`）
      完了条件: `FEATURE_FNS` の各特徴量関数を候補ごとのパラメータで差し替え可能にし（`functools.partial` 等）、fold ごとの inner 検証で複数候補を比較する。決めた値と根拠を `D-051`/`D-059` に追記する
...
- [ ] **`P-3` の完了を確認する**（`architecture.md` 7節 / `R-023`）
      ...
      **実測（2026-08-24、暫定ハイパーパラメータ〔`D-101`〕での初回実行）**: `run_p3_completion_check()` を実データ7fold（`2015-2024`、封印年除く）で実行した。所要時間 6,115秒（約102分）。
      ...
      **未達だが `P-3` の実装自体は完了。** `D-015` どおり数値を記録した。`class_weights`/カテゴリ丸め閾値が `D-101` の暫定値（未チューニング）であることが最有力の要因候補であり、上のハイパーパラメータ探索タスクが未着手のまま初回実行した結果である点に注意。`D-018`/`R-027` の年代別内訳はまだ算出していない（別途 `by_era_breakdown()` 相当の実装が要る。今回のスコープ外）
