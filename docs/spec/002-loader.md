...
|---|---|
| Phase | P-0 |
| 関連決定 | `D-005` `D-009` `D-014` `D-023` `D-026` `D-034` `D-035` `D-037` `D-038` `D-039` `D-043` `D-044` `D-045` `D-048` `D-049` `D-050` `D-057` |
...
#### 騎手・調教師 → `jockeys` / `trainers` / `runners.affiliation`

着順表の該当セルから採る（`D-057`）。**騎手ページは引かない。**

| 対象 | セルの形 | 採る値 |
|---|---|---|
| 騎手 | `<a href="/jockey/result/recent/05203/" title="岩田康誠">岩田康誠</a>` | ID `05203` と名前 `岩田康誠` |
| 調教師 | `[東]<a href="/trainer/result/recent/01145/" ...>奥村武</a>` | ID・名前・所属 `東` |

**名前は `<a>` のテキストから採る。** 調教師欄はセル先頭に所属マーカーが付くため、タグを外したセル全体を採ると名前が `[東]奥村武` になる。

所属マーカーは `[東]` `[西]` `[地]` `[外]` の4種。`runners.affiliation` に入れる（`D-049`）。
