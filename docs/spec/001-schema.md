...
|---|---|
| Phase | P-0 |
| 関連決定 | `D-009` `D-010` `D-011` `D-012` `D-020` `D-026` `D-029` `D-034` `D-035` `D-036` |
...
| 全テーブル共通列 | `source` `fetched_at` を持ち、いずれも `NOT NULL`（`D-026`） |
...
以下のDDLは DuckDB 1.4.5 で実行し、全テーブルが作成できること、および「テスト観点」の各制約が意図どおり違反を拒否することを確認済み。
...
| `corners` | コーナー通過順。**要素数はレースによって変わる**。コーナーのないコースは空配列 |

**`corners` の空配列と `NULL` を区別する（`R-015`）。** 空配列は「コーナーが存在しない」、`NULL` は「取得できていない」を表す。
...
| `出走取消` | 出ていない | あり | 数えない | `NULL` | `NULL` | **未観測**（`Q-023`） |
...
|---|---|---|---|
| `202305021211` | 日本ダービー2023 | 競走中止 | 全18行、`n_starters = 18`、`finish_pos` が非 `NULL` なのは17行 |
| `201905030611` | ユニコーンS2019 | 競走除外 | 全15行、`n_starters = 13`、`除` の2行は `finish_pos IS NULL` |
| `202007010811` | 高松宮記念2020 | 降着 | `status = '降着'` の行が1件、`finish_pos = 4`、`margin IS NULL` |
| `202009020204` | 障害4歳以上未勝利 | 同着 | `finish_pos` が `1,2,3,3,5,…`。`複勝` の払戻が4件、`ワイド` が5件 |
| `202306040911` | スプリンターズS2023 | 短距離のコーナー数 | `corners` の要素数が4未満の行が存在する |
...
| ID | 内容 | この仕様書への影響 |
|---|---|---|
| `Q-023` | 出走取消・失格の着順マーカーが未観測 | `status` の対応表が2行埋まらない。未知マーカーは取込失敗で運用上は閉じている |
...
| `Q-008` | 言語・データ処理ライブラリが未確定 | ストレージのみ `D-036` で確定。DDL はDuckDB方言に固定 |
| `Q-015` | 同一項目がページ間で食い違う場合の正 | `source` は持つが、どちらを採るかの規則は `002-loader.md` |
