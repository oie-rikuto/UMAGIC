...
前半平均     = sum(lap[1] .. lap[N-3]) / (N - 3)

f102_actual = 上がり3F平均 − 前半平均        # 単位: 秒/ハロン
```
...
**端数距離（有馬記念2500mなど）で末尾3本が600mであることは未検証（`Q-035`）。** 端数が末尾に来る場合は本節の式を見直す必要がある。
...
| 4 | ラップ本数が12本と13本のレース | どちらも1ハロンあたりに揃った値が出る（`D-087`。本数に依存しない） |
...
| ID | 内容 | この仕様書への影響 |
|---|---|---|
| `Q-035` | 端数距離のラップ本数と1本目の長さが未検証 | **`D-087` の「末尾3本＝600m」という前提が未確認。** 端数が末尾に来る場合は1節の式を見直す。実装着手前に取り込み済みデータで確認する |
