...
| 関連決定 | `D-003`, `D-012`, `D-054`, `D-058`, `D-060`, `D-086`, `D-104`, `D-105`, `D-106`, `D-107`, `D-109`, `D-110`, `D-111` |
| 関連特徴量 | `F-301`, `F-302` |
| 状態 | Draft |
...
```
y_ij = μ + β_(surface, distance_band) + γ_(race_class) + δ_(grade) + u_j + v_i + ε_ij
```
...
| `β` | 芝ダ×距離帯の固定効果 | 固定 |
| `γ` | クラスの固定効果 | 固定 |
| `δ` | グレード（`G1`/`G2`/`G3`/`L`/無し）の固定効果（`D-111`） | 固定 |
| `u_j` | レース効果（馬場差 + 展開 + `γ`/`δ` で説明されない残りのメンバーレベル） | ランダム |
| `v_i` | 馬効果（`F-302` として出力する） | ランダム |

距離帯は `D-066` の4区分を使う。`race_class` が `NULL` の行は `"不明"` という単一の水準に寄せる（`races.race_class` は7,314件が `NULL`）。`grade` が `NULL` の行は `"無し"` という単一の水準に寄せる（`races.grade` は31,581件が `NULL`）。`γ` と `δ` は独立の因子であり、`race_class='オープン'` の中に `grade` の全水準が混在する（`D-111`）。
