# 実装仕様書

`001-schema.md` のみ存在する（`Draft`）。以降は着手する Phase の分だけ追加する。

## 位置づけ

| ドキュメント | 書くこと | 書かないこと |
|---|---|---|
| `decisions.md` | なぜそうするか | 実装の詳細 |
| `domain-knowledge.md` | 何を根拠にするか | 実装の詳細 |
| **`spec/`** | **何をどう作るか** | **判断の根拠**（`D-xxx` を参照するに留める） |

仕様書に根拠を書き込まないこと。根拠は `decisions.md` に `D-xxx` として置き、仕様書からは参照する。
こうしておくと、仕様が変わっても根拠が失われず、根拠が変わったときに影響範囲を追える。

## ファイル命名

```
docs/spec/NNN-<名前>.md
```

`NNN` は3桁の連番。**取得順に振るだけで、順序や依存関係は表さない**（`D-013`）。Phase は表の列で示す。

想定される仕様書（現時点の見込み。確定ではない）。下表はPhase順に並べてあり、番号順ではない。

**この一覧は「全部書く約束」ではない。着手するPhaseの分だけ書く。** 実装コードがまだ存在せず、`Q-011`（必要なデータ項目が実際に取得できるか）も未解決の段階で全20本を確定させると、前提が崩れたときに大半が無駄になる。いま書いてよいのは `P-0` の分だけである。

| ファイル | Phase | 内容 |
|---|---|---|
| `001-schema.md` | P-0 | 中間スキーマの型定義、制約、正規化ルール |
| `002-loader.md` | P-0 | ソース抽象インターフェース、各ローダーの責務 |
| `012-data-quality.md` | P-0 | 取り込んだデータの妥当性検査。`P-0` の完了条件 |
| `019-incremental-update.md` | P-0 | 増分取得と、過去走集計の再計算範囲 |
| `020-migration.md` | P-0 | スキーマ変更時の既存DBの移行手順 |
| `003-features.md` | P-1 | 特徴量の実装仕様（`F-301` を除く。分割単位は `Q-010`） |
| `013-track-variant.md` | P-1 | 馬場差推定（`F-301`）。混合効果モデル |
| `017-feature-store.md` | P-1 | 特徴量の実体化と as-of 再現 |
| `018-cold-start.md` | P-1 | 過去走を持たない馬（外国馬・地方馬）の扱い |
| `004-leakage-test.md` | P-1 | リーク検査テストの仕様 |
| `005-baseline.md` | P-2 | ベースラインモデルの定義 |
| `014-training-pipeline.md` | P-2 | fold生成、`sample_weight`、乱数シードと再現性、モデル保存形式 |
| `006-stage1-pace.md` | P-3 | レース質予測モデル |
| `007-stage2-ranker.md` | P-3 | 適性照合モデル |
| `015-calibration.md` | P-3 | G1での確率校正（`D-003` の3番目の手段） |
| `008-probability.md` | P-4 | Plackett-Luce展開 |
| `009-betting.md` | P-4 | 期待値計算と賭け金決定 |
| `010-backtest.md` | P-4 | 検証手順とレポート様式 |
| `011-cli.md` | P-5 | CLIのコマンド体系 |
| `016-inference.md` | P-5 | 当日の予測実行フロー |

## テンプレート

```markdown
# NNN <タイトル>

| | |
|---|---|
| Phase | P-x |
| 関連決定 | D-xxx, D-xxx |
| 関連特徴量 | F-xxx, F-xxx |
| 状態 | Draft / Fixed / Implemented |

## 目的

このモジュールが何を担うか。1〜3文。

## 入出力

入力と出力の型・形式を明記する。

## 仕様

実装すべき内容。曖昧さが残らない粒度で書く。

## 制約

守るべき条件（リーク防止、性能要件など）。

## テスト観点

何を検証すれば「できた」と言えるか。

## 未決事項

この仕様書の範囲で未確定のもの。`Q-xxx` として `open-questions.md` にも登録する。
```
