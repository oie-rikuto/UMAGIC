...
def _parse_header(text: str, race_id: int) -> dict:
    ...
    if span_m:
        ...
        #
        # 方向のあとにコース形状が挟まる。保存済み587ページの実測では
        # 「芝右」「芝左」「ダ右」「ダ左」「芝右 外」「芝右 外-内」の6通り。
        # 「外-内」は阪神・京都の3200m（外回りから内回りへ）で、`外` か `内`
        # の一方しか見ない正規表現では松籟S（阪神芝3200m）が読めなかった。
        # 形状の語を数え上げるとまた漏れるので、数字までの非数字を読み飛ばす。
        # 区切りの `/` を跨がせないことで、距離が無いときに天候側の数字を
        # 拾わないようにする。
        #
        ...
        sd_m = re.match(r"(芝|ダ)(右|左|直線)?[^0-9]{0,10}(\d+)m", head)
        if sd_m:
...
_STATUS_MAP: dict[str, str] = {"中": "競走中止", "除": "競走除外"}
