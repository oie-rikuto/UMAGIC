"""`F-704` 騎手のコース適性（`docs/spec/003-features.md` / `D-066` / `D-067`）。

`F-202` と同じ形（条件別成績の縮約）で、`jockey_id` × コース・距離帯
（`D-066` の4区分）の成績（`D-067`: `finish_pos / n_starters`）を縮約する。
`F-202` と異なり条件に `course`（開催競馬場）を使う（馬場状態ではなく）。
"""
...
SELECT t.race_id, t.horse_id, r.date, r.course,
...
_POPULATION_SQL = f"""
SELECT
    ru.jockey_id, r.date, r.course,
    {_DISTANCE_BAND_CASE} AS distance_band,
    CAST(ru.finish_pos AS DOUBLE) / r.n_starters AS perf
FROM runners ru
JOIN races r USING (race_id)
WHERE ru.finish_pos IS NOT NULL AND ru.jockey_id IS NOT NULL
"""
...
def compute_f704(
    ...
    def _shrink_row(s: dict) -> float:
        ...
        return shrink(n, s["f704_sum"] / n, k=k, mu_global=s["mu_global_c"])

    out = out.with_columns(
        pl.struct(["f704_n", "f704_sum", "mu_global_c"])
        .map_elements(_shrink_row, return_dtype=pl.Float64)
        .alias("f704")
    )

    out = out.with_columns(
        pl.when(pl.col("jockey_id").is_null()).then(None).otherwise(pl.col("f704")).alias("f704"),
        pl.when(pl.col("jockey_id").is_null()).then(1).otherwise(0).alias("f704_unavailable"),
    )

    return out.select(_KEY_COLS + ["f704", "f704_unavailable"])
